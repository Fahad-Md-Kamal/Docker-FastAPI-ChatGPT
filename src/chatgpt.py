...
def ask_pychatgpt1(ask):
    # session_token = token  # `__Secure-next-auth.session-token` cookie from https://chat.openai.com/chat
    api1 = ChatGPT(session_token=os.getenv("CHATGPT-TOKEN", "NO TOKEN"))  # auth with session token
    ...
    #                verbose=True)
    # reuse cookies generated by successful login before login, if `login_cookies_path` does not exist,
    # it will process logging  with `auth_type`, and save cookies to `login_cookies_path` only works when
    # `auth_type` is `openai` or `google`
    # api9 = ChatGPT(auth_type='openai', email='example@xxx.com', password='password',
    #                login_cookies_path='your_cookies_path',
    #                verbose=True)

    resp = api1.send_message(ask)
